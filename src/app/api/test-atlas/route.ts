...
export async function GET(request: NextRequest) {
    try {
        ...
        const testCollections = ['categories', 'theories', 'questions', 'flashcards', 'summaries'];
        
        for (const collectionName of testCollections) {
            try {
                const collection = mongoose.connection.db!.collection(collectionName);
                const count = await collection.countDocuments();
                const sample = count > 0 ? await collection.findOne({}, { projection: { title: 1, name: 1, question: 1, _id: 0 } }) : null;
                
                stats[collectionName] = {
                    count,
                    sample: sample ? (sample.title || sample.name || sample.question || 'N/A') : null
                };
            } catch (err) {
                stats[collectionName] = {
                    count: 0,
