            ...
            if (errorElement > 0) {
                ...
                if (
                    ...
                    (errorText.toLowerCase().includes("error") ||
                        errorText.toLowerCase().includes("erro"))
                ) {
                    return true;
                }
            }
...
test.describe("📱 Frontend Public Pages - Complete Journey Tests", () => {
    ...
    test("📚 Categories Page - Complete Flow", async ({ page }) => {
        ...
        await test.step("Validate categories display", async () => {
            ...
            const categories = page.locator(
                '[data-testid*="category"], .category-card, .category-item'
            );
    ...
    test("🧮 Theory Pages - LaTeX and Content", async ({ page }) => {
        ...
        await test.step("Test theory content rendering", async () => {
            const theoryItems = page.locator(
                '[data-testid*="theory"], .theory-card, .theory-item'
            );
            ...
            if (theoryCount > 0) {
                ...
                const content = page.locator(
                    '[data-testid="theory-content"], .theory-content, .markdown-content'
                );
    ...
    test("❓ Questions and Practice", async ({ page }) => {
        ...
        await test.step("Validate question interactions", async () => {
            const questions = page.locator(
                '[data-testid*="question"], .question-card, .question-item'
            );
...
test.describe("👑 Admin Panel - Complete CRUD Testing", () => {
    ...
    test("📚 Theory Admin - Full CRUD Cycle", async ({ page }) => {
        ...
        await test.step("Test theory listing", async () => {
            ...
            const theoryTable = page.locator(
                'table, .content-table, [data-testid="theory-list"], [class*="table"]'
            );
            expect(await theoryTable.count()).toBeGreaterThan(0);

            // Test filters if present
            const filters = page.locator(
                '[data-testid*="filter"], input[placeholder*="Buscar"], select, [class*="select"]'
            );
            if ((await filters.count()) > 0) {
                const searchInput = page
                    .locator('input[placeholder*="Buscar"], input[type="text"]')
                    .first();
        ...
        await test.step("Test theory creation form", async () => {
            ...
            const titleInput = page
                .locator(
                    'input[name="title"], input[placeholder*="título"], input[placeholder*="Title"], input[type="text"]'
                )
                .first();
            const contentTextarea = page
                .locator(
                    'textarea[name="content"], .markdown-editor textarea, textarea'
                )
                .first();
            ...
            if (
                ...
                const previewButton = page.locator(
                    'button:has-text("Preview"), button:has-text("Visualizar"), button:has-text("preview")'
                );
    ...
    test("❓ Questions Admin - 5 Alternatives Validation", async ({ page }) => {
        ...
        await test.step("Test question creation form", async () => {
            ...
            const alternatives = page.locator(
                'input[name*="alternative"], input[placeholder*="Alternativa"]'
            );
            ...
            if ((await questionInput.count()) > 0) {
                ...
                if (altCount > 0) {
                    ...
                    const correctRadios = page.locator(
        ...
        if (isMobile) {
            await test.step("Test mobile navigation", async () => {
                ...
                const hamburger = page.locator(
                    '[data-testid="hamburger"], .hamburger, .mobile-menu-trigger'
                );
                if ((await hamburger.count()) > 0) {
                    await hamburger.click();
                    await utils.waitForLoading();
