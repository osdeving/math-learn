...
async function migrateTheories(categoryMap) {
    ...
    const mongoTheories = await Theory.find({}).lean();
    ...
    for (const theory of mongoTheories) {
        const prismaTheory = await prisma.theory.create({
            data: {
                ...
                isPublished: theory.isPublished || false,
                createdAt: theory.createdAt
                    ? new Date(theory.createdAt)
                    : new Date(),
                updatedAt: theory.updatedAt
                    ? new Date(theory.updatedAt)
                    : new Date(),
            },
        });

        // Create category relationships
        if (theory.categoryIds && Array.isArray(theory.categoryIds)) {
...
async function migrateSummaries(categoryMap) {
    ...
    const mongoSummaries = await Summary.find({}).lean();
    ...
    for (const summary of mongoSummaries) {
        ...
        // Create category relationships
        if (summary.categoryIds && Array.isArray(summary.categoryIds)) {
...
async function migrateFlashcards(categoryMap) {
    ...
    const mongoFlashcards = await Flashcard.find({}).lean();
    ...
    for (const flashcard of mongoFlashcards) {
        const prismaFlashcard = await prisma.flashcard.create({
            data: {
                ...
                isPublished: flashcard.isPublished || false,
    ...
    for (const question of mongoQuestions) {
        const prismaQuestion = await prisma.question.create({
            data: {
                ...
                statement: question.statement || "Enunciado não disponível",
        ...
        if (question.alternatives && Array.isArray(question.alternatives)) {
            for (let i = 0; i < question.alternatives.length; i++) {
                ...
                await prisma.alternative.create({
                    data: {
                        text: alternative.text || `Alternativa ${i + 1}`,
        ...
        // Create category relationships
        if (question.categoryIds && Array.isArray(question.categoryIds)) {
        ...
        console.log(
            `✅ Migrated question: ${question.title || "Sem título"} (${
                question._id
            } → ${prismaQuestion.id})`
        );
    }
